...
meta = [
  ...
  "argument_groups" : [
    {
      ...
      "arguments" : [
        ...
        },
        {
          "type" : "double",
          "name" : "--scale",
          "description" : "\\"Scale parameter, which suggest approximate cell radius for the algorithm. Must be in the same units as \nx and y molecule coordinates. Negative values mean it must be estimated from `min_molecules_per_cell`.\\"\n",
          "default" : [
            -1.0
          ],
          "required" : false,
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_spatial_segmentation/task_spatial_segmentation/src/methods/baysor/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker|native",
    "output" : "target/nextflow/methods/baysor",
    "viash_version" : "0.9.7",
...
par = {
  ...
  'prior_segmentation_key': $( if [ ! -z ${VIASH_PAR_PRIOR_SEGMENTATION_KEY+x} ]; then echo "r'${VIASH_PAR_PRIOR_SEGMENTATION_KEY//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  ...
  'prior_segmentation_confidence': $( if [ ! -z ${VIASH_PAR_PRIOR_SEGMENTATION_CONFIDENCE+x} ]; then echo "float(r'${VIASH_PAR_PRIOR_SEGMENTATION_CONFIDENCE//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi )
}
meta = {
...
BAYSOR_OUTPUT_DIR = TMP_DIR / "baysor_output"
BAYSOR_OUTPUT_SEG_FILE = BAYSOR_OUTPUT_DIR / "segmentation.csv"
...
df_baysor = pd.read_csv(BAYSOR_OUTPUT_SEG_FILE)
...
def convert_str_ids_to_ints(df, file_path_for_error_messages=None):
    ...
    if (len(unique_types) == 1) and (str in unique_types):
        df.loc[~df["cell"].isnull(), "cell"] = (
            df.loc[~df["cell"].isnull(), "cell"]
            .apply(lambda i: re.split(r"[-_]", i)[-1])
            .astype(int)
        )
...
df_baysor = convert_str_ids_to_ints(df_baysor, file_path_for_error_messages=BAYSOR_OUTPUT_SEG_FILE)
...
sdata_transcripts_only = sd.SpatialData(
    labels={'segmentation': sdata.labels[par['prior_segmentation_key']]},
    points={
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_spatial_segmentation/methods/baysor",
    "tag" : "build_segger"
  },
  "label" : [
    "hightime",
